...
For example, this is a complete, valid import for an Ethernet capture and
...
`Ether` and `IP` are packet classes, `conf` is Scapy's live configuration
object, and the remaining names are functions. Import only the names a script
actually uses.

<a id="core-api-map"></a>

## [Core API map](#contents "Back to contents")

`scapy.all` exports a large convenience namespace rather than a small, stable
list of functions. The exact names depend on the Scapy version, operating
system, optional dependencies, and loaded protocol modules. The tables below
cover the general-purpose public API used in packet scripts; use the discovery
commands at the end of this section for the exhaustive list in an installed
version.

### Packet classes and value generators

| Area | Common names | Purpose |
|---|---|---|
| Link layer | `Ether`, `ARP`, `Dot1Q`, `Dot1AD`, `LLC`, `SNAP` | Ethernet, address resolution, and VLAN framing |
| Internet layer | `IP`, `IPv6`, `ICMP`, `ICMPv6EchoRequest`, `ICMPv6EchoReply` | IPv4, IPv6, and control messages |
| Transport | `TCP`, `UDP`, `SCTP` | Transport headers |
| Common application protocols | `DNS`, `DNSQR`, `DNSRR`, `DHCP`, `BOOTP` | Frequently used protocol records; other modules are discoverable with `explore()` |
| Payload | `Raw`, `Padding` | Explicit bytes and decoded padding |
| Packet base types | `Packet`, `NoPayload` | Custom packet definitions and an empty payload marker |
| Variable values | `RandShort`, `RandInt`, `RandIP`, `RandMAC`, `RandString` | Values generated when packets are iterated or built |

Packet classes are constructors, not send functions. Creating `Ether()/IP()`
does not place anything on the network.

### Build, inspect, and dissect

| Function or method | Purpose | Typical result |
|---|---|---|
| `raw(pkt)` / `bytes(pkt)` | Assemble a packet into wire bytes | `bytes` |
| `hexdump(pkt)` | Print a classic hex/ASCII dump | Printed output |
| `hexstr(pkt)` | Return a compact hexadecimal string | `str` |
| `pkt.summary()` | Return a one-line packet description | `str` |
| `pkt.show()` | Print decoded fields as currently represented | Printed output |
| `pkt.show2()` | Assemble a copy, then show calculated fields | Printed output |
| `pkt.sprintf(format)` | Format selected packet fields | `str` |
| `ls()` / `ls(Layer)` | List loaded packet classes or fields | Printed output |
| `rfc(Layer)` | Draw a packet class's bit layout | Printed output |
| `explore()` | Browse built-in and contributed protocol modules | Interactive output |
| `fragment(pkt, fragsize=...)` | Split IPv4 packets into fragments without sending | `list[Packet]` |
| `defragment(pkts)` | Reassemble compatible IPv4 fragments | Packet collections |
| `bind_layers(lower, upper, **fields)` | Bind classes for both building and dissection | `None` |
| `bind_bottom_up(...)` | Add a dissection-only binding | `None` |
| `bind_top_down(...)` | Add a construction-only binding | `None` |
| `split_layers(lower, upper, **fields)` | Remove a matching two-way binding | `None` |

### Send, receive, and capture

| Function or class | Layer | Purpose |
|---|---:|---|
| `send()` | 3 | Send packets; Scapy selects the route and link layer |
| `sendp()` | 2 | Send caller-built link-layer frames on an interface |
| `sr()` / `sr1()` | 3 | Send and receive all matched answers / the first answer |
| `srp()` / `srp1()` | 2 | Layer 2 equivalents of `sr()` / `sr1()` |
| `sniff()` | 2+ | Capture live packets or process an offline capture |
| `AsyncSniffer` | 2+ | Start, stop, and join capture from program logic |
| `srloop()` / `srploop()` | 3 / 2 | Repeat a bounded request/response operation interactively |
| `sendpfast()` | 2 | Replay through `tcpreplay`; requires that external tool |
| `bridge_and_sniff()` | 2 | Forward between two interfaces while capturing |
| `sendflood()` / `srflood()` / `sr1flood()` / `srpflood()` | 3 / 2 | High-rate helpers; use only in an isolated, specifically authorized load test |

For normal scripts, prefer `send`, `sendp`, `sr*`, and `sniff` with finite
`count`, `timeout`, `inter`, and `retry` values. `bridge_and_sniff()` changes
traffic flow, and the flood helpers are deliberately not demonstrated here.

### Interfaces, addressing, routes, and configuration

| Name | Purpose | Return or important behavior |
|---|---|---|
| `conf` | Global runtime configuration | Holds `iface`, `ifaces`, `route`, `route6`, socket classes, verbosity, and feature flags |
| `get_if_list()` | List interface identifiers accepted by Scapy | `list[str]` |
| `get_if_addr(iface)` | Get an interface's primary IPv4 address | IPv4 address string; commonly `"0.0.0.0"` when unavailable |
| `get_if_hwaddr(iface)` | Get an interface's hardware/MAC address | MAC-address string or a platform-specific error if unsupported |
| `getmacbyip(ip)` | Resolve the next-hop MAC used to reach an IPv4 destination | MAC string or `None`; may issue an ARP request |
| `get_working_if()` | Select one usable Scapy interface | `NetworkInterface` or `None` |
| `get_working_ifaces()` | List usable Scapy interfaces | `list[NetworkInterface]` |
| `resolve_iface(name)` | Resolve a name into Scapy's interface object | `NetworkInterface` |
| `show_interfaces()` / `conf.ifaces.show()` | Print Scapy's interface table | Printed output |
| `conf.route.route(ip)` | Resolve an IPv4 route | `(iface, source_ip, gateway)` |
| `conf.route6.route(ip)` | Resolve an IPv6 route | Route tuple; details are platform/version dependent |

`getmacbyip()` resolves the link-layer **next hop**, not necessarily the target
host. For an off-link destination it normally returns the gateway's MAC; for
an on-link unicast destination it consults Scapy's cache and may transmit an
ARP who-has. It is therefore not a purely passive lookup.

### Capture-file I/O and packet collections

| Name | Purpose |
|---|---|
| `rdpcap()` | Read a complete PCAP or PCAPNG packet collection |
| `wrpcap()` | Write packets to a PCAP file |
| `wrpcapng()` | Write PCAPNG explicitly |
| `PcapReader` / `PcapNgReader` | Stream packets from large PCAP / PCAPNG files |
| `PcapWriter` / `PcapNgWriter` | Incrementally write PCAP / PCAPNG files |
| `PacketList` | General packet collection with `summary`, `nsummary`, `filter`, and plotting helpers |
| `SndRcvList` | Collection of matched `(sent, received)` pairs |

### Loading, diagnostics, and exhaustive discovery

| Name | Purpose |
|---|---|
| `load_layer("http")` | Load an additional built-in protocol module |
| `load_contrib("mqtt")` | Load an optional contributed protocol module |
| `list_contrib()` | List contributed modules known to the installation |
| `lsc()` | List Scapy's interactive commands |
| `help(name)` | Show the Python signature and documentation for one name |
| `conf.commands` | Registry of Scapy commands currently loaded |
| `conf.layers` | Registry of packet classes currently loaded |

To enumerate the exact convenience API available in the running environment:

```python
import inspect
import scapy.all as scapy

public = sorted(name for name in dir(scapy) if not name.startswith("_"))
functions = [name for name in public if inspect.isfunction(getattr(scapy, name))]
classes = [name for name in public if inspect.isclass(getattr(scapy, name))]
objects = [name for name in public if name not in set(functions + classes)]

print("functions:", *functions, sep="\n  ")
print("classes:", *classes, sep="\n  ")
print("other objects:", *objects, sep="\n  ")
```

This is more accurate than a frozen list: imports from `scapy.all`, plugins,
and platform backends can add or omit names. Follow with `help(scapy.name)` to
inspect a particular callable's current signature and caveats.
...
| Argument | Meaning |
|---|---|
| `x` | One packet, an iterable, a `PacketList`, or a packet generator |
| `iface` | Output interface for Layer 2 functions such as `sendp`; Layer 3 normally follows the route |
| `count` | Send the packet set this many times; the default is once |
| `inter` | Delay in seconds between packets |
| `loop` | Repeat continuously when nonzero; avoid in ordinary scripts |
| `realtime` | Pace packets using their timestamps where supported |
| `verbose` | Override `conf.verb` for this call |
| `return_packets` | Make `send()` or `sendp()` return the packets sent |

The `sr*` family additionally accepts `timeout`, `retry`, `multi`, and
`threaded`. `timeout` bounds how long Scapy waits after sending; `retry`
resends unanswered probes; `multi=True` permits more than one answer to match
a request. Check `help(sr)` or `help(srp)` for the installed version's full
signature.
...
| Get an interface's IPv4 address | `get_if_addr("eth0")` |
| Get an interface's MAC address | `get_if_hwaddr("eth0")` |
| Resolve the next-hop MAC for IPv4 | `getmacbyip("192.0.2.10")` |
...
for name in get_if_list():
    print(name, get_if_addr(name), get_if_hwaddr(name))

# This may send ARP and returns the next-hop MAC, or None on failure.
next_hop_mac = getmacbyip("192.0.2.10")
print(f"next-hop MAC={next_hop_mac or 'unresolved'}")
```
